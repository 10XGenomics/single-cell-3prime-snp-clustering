...
def compute_se_3p_insert_size(read, gene_index):
    """ Single-end, 3' insert size """

    insert_sizes = []
    for transcript, strand, pos, _ in cr_utils.get_read_transcripts_iter(read):
        if strand == cr_constants.FORWARD_STRAND:
            transcript_length = gene_index.get_transcript_length(transcript)
            insert_size_for_transcript = transcript_length - pos
        else:
            insert_size_for_transcript = pos
        if insert_size_for_transcript <= cr_constants.MAX_INSERT_SIZE:
            insert_sizes.append(insert_size_for_transcript)

    if len(insert_sizes) > 0:
        return np.median(insert_sizes)
    return None

def compute_pe_insert_size(read, transcript_alignments):
    """ Paired-end insert size """

    insert_sizes = []
    for transcript_alignment in transcript_alignments:
        if transcript_alignment.is_read2:
            continue
        insert_size_for_transcript = abs(transcript_alignment.tlen)
        if insert_size_for_transcript <= cr_constants.MAX_INSERT_SIZE:
            insert_sizes.append(insert_size_for_transcript)
...
def infer_cdr3_impurity(obs_multi_cdr3_rate, cdr3_det_rate):
    """ Infer the fraction of cells w/ >1 CDR3, taking into account sensitivity.
        This assumes the detection rate is uniform across all cells.
        Args: observed_multi_cdr3_rate: Fraction of cells where >1 CDR3 was detected
              cdr3_det_rate:            Fraction of cells where >0 CDR3s were detected """
    # TODO
    pass
