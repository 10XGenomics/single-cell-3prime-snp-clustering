...
class GeneBCMatrix:
    ...
    @staticmethod
    def preprocess_matrix(matrix, num_bcs=None, use_bcs=None):
        if use_bcs is not None:
            bc_seqs = [x.strip() for x in open(use_bcs, 'r')]
            bc_indices = [matrix.bc_to_int(bc) for bc in bc_seqs]
...
def concatenate_mex_dirs(mex_dir_list, out_mex_dir):
    ...
    shutil.copytree(mex_dir_list[0], out_mex_dir)
