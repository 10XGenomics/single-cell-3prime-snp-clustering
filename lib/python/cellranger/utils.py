...
def si_pcr_dupe_func(read):
    return cdna_pcr_dupe_func(read), read.tid, read.pos
...
def get_read_transcripts_iter(read):
    ...
    for x in s.split(';'):
        ...
        assert strand in cr_constants.STRANDS

        yield chrom, strand, pos, cigarstring

def _make_transcripts_tag(transcript_alignments):
    entries = []
    for transcript_alignment in transcript_alignments:
        entry = ','.join([
            transcript_alignment.transcript,
            transcript_alignment.strand + str(transcript_alignment.pos),
            transcript_alignment.cigarstring,
        ])
        entries.append(entry)
    return ';'.join(entries)

def make_genes_and_transcripts_tags(transcriptome_reads, transcripts, gene_index, strandedness):
    read_transcript_alignments = []
    read_genes = collections.OrderedDict()

    any_antisense = False
    for read in transcriptome_reads:
        if read.is_unmapped:
            continue
        if not is_transcriptome_read_sense(read, strandedness):
            any_antisense = True
            continue

        transcript = transcripts[read.tid]
        strand = '-' if read.is_reverse else '+'
        read_transcript_alignments.append(cr_constants.TranscriptAlignment(
            transcript, strand, read.pos, read.cigarstring))

        gene = gene_index.get_gene_from_transcript(transcript)
        assert gene is not None
        if gene.id not in read_genes:
            read_genes[gene.id] = gene.name

    tags = []
    if len(read_transcript_alignments) > 0:
        tags.append((cr_constants.TRANSCRIPTS_TAG, _make_transcripts_tag(read_transcript_alignments)))

    if len(read_genes) > 0:
        tags.append((cr_constants.GENE_IDS_TAG, ';'.join(read_genes.keys())))
        tags.append((cr_constants.GENE_NAMES_TAG, ';'.join(read_genes.values())))

    return tags, any_antisense, read_genes

def genome_and_transcriptome_bams_iter(in_genome_bam, in_transcriptome_bam, in_trimmed_bam):
    # Iterate through genome and transcriptome BAM by qname simultaneously
    # Accounts for reads missing from the transcriptome BAM
    # Assume the trimmed-read-bam has every qname in the genome bam, in the same order.
    # The trimmed input bam is optional

    genome_bam_iter = itertools.groupby(in_genome_bam, key=lambda read: read.qname)
    transcriptome_bam_iter = itertools.groupby(in_transcriptome_bam, key=lambda read: read.qname)
    ...
    for (genome_qname, genome_reads), trimmed_tuple in itertools.izip_longest(genome_bam_iter,
        ...
        transcriptome_qname, transcriptome_reads = next(transcriptome_bam_iter,
                                                        (None, []))
        while transcriptome_qname != genome_qname:
            yield (genome_qname, genome_reads, [], trimmed_reads)
            genome_qname, genome_reads = genome_bam_iter.next()
            trimmed_qname, trimmed_reads = next(trimmed_bam_iter, (None, []))
        yield (genome_qname, genome_reads, transcriptome_reads, trimmed_reads)
...
def is_read_mapped(read, region, gene_index=None, chroms=None):
    is_mapped = not read.is_unmapped
    if region == cr_constants.TRANSCRIPTOME_REGION:
        return is_mapped and get_read_gene_ids(read) is not None
    elif region == cr_constants.GENOME_REGION:
        return is_mapped
    elif region == cr_constants.EXONIC_REGION:
        assert gene_index is not None and chroms is not None
        return is_mapped and (
            get_read_gene_ids(read) is not None or \
            gene_index.is_read_exonic(read, chroms))
    elif region == cr_constants.INTERGENIC_REGION:
        assert gene_index is not None and chroms is not None
        return is_mapped and get_read_gene_ids(read) is None and \
            not gene_index.is_read_exonic(read, chroms) and \
            gene_index.is_read_intergenic(read, chroms)
    elif region == cr_constants.INTRONIC_REGION:
        assert gene_index is not None and chroms is not None
        return is_mapped and get_read_gene_ids(read) is None and \
            not gene_index.is_read_exonic(read, chroms) and \
            gene_index.is_read_intronic(read, chroms)
    else:
        raise Exception('Invalid region: %s' % region)

def is_read_conf_mapped(read, region, high_conf_mapq, gene_index=None, chroms=None):
    is_conf_mapped = is_read_mapped(read, region, gene_index=gene_index, chroms=chroms) and read.mapq >= high_conf_mapq
    if region == cr_constants.TRANSCRIPTOME_REGION:
        return is_conf_mapped and len(get_read_gene_ids(read)) == 1
    else:
        return is_conf_mapped
...
def is_read_dupe_candidate(read, high_conf_mapq, use_corrected_umi=True):
    ...
    return not read.is_secondary and umi and get_read_barcode(read) and \
        is_read_conf_mapped(read, cr_constants.TRANSCRIPTOME_REGION, high_conf_mapq)
...
def is_read_conf_mapped_to_transcriptome_deduped(read, high_conf_mapq):
    return not read.is_secondary and not read.is_duplicate and \
        is_read_conf_mapped(read, cr_constants.TRANSCRIPTOME_REGION, high_conf_mapq) and \
